...
int main(int argc, char* argv[])
{
    ...
    StrassenMultMPI(firstMatrix, secondMatrix, resultMatrix,
                       dim, my_rank, comm_size, startNode);
    endTime = MPI_Wtime();
                        ...
                        int dim, int my_rank, int comm_size, int startNode)
{
    ...
    if (dim > 2)
    {
            ...
            else
            {
                ...
                if (my_rank == startNode)
                    ...
                    MPI_Recv(m1, subDim*subDim, MPI_DOUBLE, 
                             (7+(7*startNode)), 0, MPI_COMM_WORLD, &status);
            }
        }
            ...
            else
            {
                ...
                {   // Receive m2 from next-level startNode
                    MPI_Recv(m2, subDim*subDim, MPI_DOUBLE,
                             (7+(7*(startNode+1))), 0, MPI_COMM_WORLD, &status);
            ...
            else
            {
                ...
                if (my_rank == (startNode+2))
                {
                    ...
                    MPI_Recv(m3, subDim*subDim, MPI_DOUBLE,
                             (7+(7*(startNode+2))), 0, MPI_COMM_WORLD, &status);
            ...
            else
            {
                 ...
                 if (my_rank == (startNode+3))
                {
                    ...
                    MPI_Recv(m4, subDim*subDim, MPI_DOUBLE,
                             (7+(7*(startNode+3))), 0, MPI_COMM_WORLD, &status);
                 ...
                 if (my_rank != (startNode+4))    // only next-level nodes should do recursive call
                 {
                  ...
                  if (my_rank == (startNode+4))
                  {
                      ...
                      MPI_Recv(m5, subDim*subDim, MPI_DOUBLE,
                               (7+(7*(startNode+4))), 0, MPI_COMM_WORLD, &status);
            ...
            else
            {
                ...
                if (my_rank == (startNode+5))
                {
                      ...
                      MPI_Recv(m6, subDim*subDim, MPI_DOUBLE,
                               (7+(7*(startNode+5))), 0, MPI_COMM_WORLD, &status);
            ...
            else
            {
                ...
                if (my_rank == (startNode+6))
                {
                      ...
                      MPI_Recv(m7, subDim*subDim, MPI_DOUBLE,
                               (7+(7*(startNode+6))), 0, MPI_COMM_WORLD, &status);
        ...
        if (my_rank == startNode)
        {
            // Receive m2-m7
            MPI_Recv(m2, subDim*subDim, MPI_DOUBLE, 
                     (startNode+1), 0, MPI_COMM_WORLD, &status);

            MPI_Recv(m3, subDim*subDim, MPI_DOUBLE, 
                     (startNode+2), 0, MPI_COMM_WORLD, &status);

            MPI_Recv(m4, subDim*subDim, MPI_DOUBLE, 
                     (startNode+3), 0, MPI_COMM_WORLD, &status);

            MPI_Recv(m5, subDim*subDim, MPI_DOUBLE, 
                     (startNode+4), 0, MPI_COMM_WORLD, &status);

            MPI_Recv(m6, subDim*subDim, MPI_DOUBLE, 
                     (startNode+5), 0, MPI_COMM_WORLD, &status);

            MPI_Recv(m7, subDim*subDim, MPI_DOUBLE, 
                     (startNode+6), 0, MPI_COMM_WORLD, &status);
